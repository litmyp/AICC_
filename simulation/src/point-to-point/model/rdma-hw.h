...
// lty: 共享内存相关头文件
#include <atomic>
#include <mutex>  // lty 用于 std::once_flag
...
// lty: 在共享内存中保存rtt的数据结构
struct RttShmData {
	std::atomic<uint64_t> sequence;  // 序列号，用于标识数据更新
	uint32_t node_id;
	uint32_t sip;
	uint16_t sport;
	uint32_t dip;
...
class ShmManager {
	private:
		...
		static RttShmData* rtt_data;//rtt信息
		static int shm_fd;
		static std::once_flag init_flag;
		// lty: 为每个node_id维护独立的sequence计数器
		static std::unordered_map<uint32_t, uint64_t> node_sequence_map;
		static std::mutex sequence_map_mutex;  // 保护map的并发访问
		
		static void InitShm();//初始化共享内存
		
	public:
		static RttShmData* GetShm() {//确保只init一次
			std::call_once(init_flag, InitShm);
			return rtt_data;
		}
...
public:
	...
	Time m_ackFlushTimeout;	//ACK刷新定时器超时时间
	...
	void ScheduleAckFlush(Ptr<RdmaRxQueuePair> q);
	void HandleAckFlushTimeout(Ptr<RdmaRxQueuePair> q);
	void SendReceiverFeedback(Ptr<RdmaRxQueuePair> q, uint32_t seq, bool isAck, bool setCnp, const IntHeader &ih);
